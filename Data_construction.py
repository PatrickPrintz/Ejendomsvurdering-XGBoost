            ...
            if data and isinstance(data, list) and len(data) > 0:
                for i in range(len(data)):
                    ...
                    if boligtype in ['1', '2', '4'] and int(enhed_anv) < 149:
                        data_entry = data[i]
                        # Check if 'BOLIGTYPE_KODE' is present in the entry
                        boligtype = data_entry.get('BOLIGTYPE_KODE', None)
                        enhed_anv = data_entry.get('ENH_ANVEND_KODE', None)
                        # Check if 'bygning' is present in the entry
                        bygning = data_entry.get('bygning', None)
                        if bygning:
                            # Check if the required keys are present in the 'bygning' dictionary
                ...
                # If no entry with boligtype equal to 1 is found, print a message and return None
...
# Write the results to CSV every 100 addresses
with concurrent.futures.ThreadPoolExecutor() as executor:
    count = 0
    with open(csv_file_path, 'a', newline='') as csv_file:
        csv_writer = csv.writer(csv_file)
            ...
            if data and isinstance(data, list) and len(data) > 0:
                ...
                if matrikel_nr:
                  url = f"https://services.datafordeler.dk/MATRIKEL/Matrikel/1/REST/SamletFastEjendom?username=TAKWDMRRWE&password=Patrick!1&Format=JSON&SFEBFEnr={matrikel_nr}"
                    ...
                    # Use a context manager for the second session
...
df.info()
# Data cleaning
df['discount'] = df['discount'].str.replace('%', '')  # Remove percentage signs
df['discount'] = pd.to_numeric(df['discount'], errors='coerce').fillna(0)  # Convert to numeric, coerce errors to NaN, fill NaN with 0
    ...
    try:
        with session.get(add) as response:
            ...
            if data:
                ...
                try:
                    result = [address, None, None, None]  # Define result with a default value
                ...
                # If data is empty, set a default result
                result = [address, None, None, None]
            return result
...
# Pre-compute unions outside the loop
...
# Formula for approximating distance between two points on earth
def distance(lat1, lon1, lat2, lon2):
    R = 6371  # radius of the earth in km
    lat1, lon1, lat2, lon2 = map(np.radians, [lat1, lon1, lat2, lon2])
    x = (lon2 - lon1) * np.cos(0.5 * (lat2 + lat1))
